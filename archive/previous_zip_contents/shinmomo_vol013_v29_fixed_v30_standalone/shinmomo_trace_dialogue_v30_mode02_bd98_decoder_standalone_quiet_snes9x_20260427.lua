-- shinmomo_trace_dialogue_v30_mode02_bd98_decoder_standalone_quiet_snes9x_20260427.lua
-- v30: standalone quiet logger. C0:BD98 mode02 bitstream decoder + source-state pointer scan
--
-- 目的:
...
--
-- 出力:
--   TRACE_DIALOGUE_V30_MODE02_BEST
--   TRACE_DIALOGUE_V30_MODE02_CANDIDATES
--   TRACE_DIALOGUE_V30_ALIVE
...
logLine("shinmomo dialogue v30 standalone loaded. BD98 mode02 + source-state pointer scan. small kana: F5=っ F6=ゃ F7=ゅ F8=ょ")
...
logLine("TRACE_DIALOGUE_V30_READY,frame=" .. emu.framecount() .. "," .. state())
...
while true do
  ...
  if TRACE_ALIVE_FRAMES > 0 and (f % TRACE_ALIVE_FRAMES) == 0 then
    logLine("TRACE_DIALOGUE_V30_ALIVE,frame=" .. f .. "," .. state() .. ",source_state=" .. source_state_words())
  end
  ...
  if active_text() then
      ...
      if best ~= nil and best.score >= MIN_GOOD_SCORE then
        ...
        if best_key ~= last_mode02_key and (f - last_mode02_frame) > MODE02_DECODE_MIN_GAP then
          ...
          if TRACE_VERBOSE_EVENTS then
            logLine(table.concat({
              "TRACE_DIALOGUE_V30_MODE02_EVENTS",
              "frame=" .. f,
              "source=" .. best.name,
              "ptr=" .. ptr_text(best.bank, best.addr),
    ...
    if TRACE_SCREEN_OBSERVED or TRACE_TOKENS then
      ...
      if TRACE_SCREEN_OBSERVED and decoded_buf ~= "" and (f - last_display_frame) > DISPLAY_IDLE_FLUSH_FRAMES then
        logLine(table.concat({
          "TRACE_DIALOGUE_V30_SCREEN_LINE",
          "frame=" .. f,
