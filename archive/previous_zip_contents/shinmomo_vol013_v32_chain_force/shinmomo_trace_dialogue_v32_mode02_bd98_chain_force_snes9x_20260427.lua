-- shinmomo_trace_dialogue_v32_mode02_bd98_chain_snes9x_20260427.lua
-- v32: standalone quiet logger. C0:BD98 mode02 bitstream decoder + chained stream scan
--
-- 目的:
...
--
-- 出力:
--   TRACE_DIALOGUE_V32_MODE02_BEST
--   TRACE_DIALOGUE_V32_MODE02_CANDIDATES
--   TRACE_DIALOGUE_V32_ALIVE
...
logLine("shinmomo dialogue v32 standalone loaded. BD98 mode02 chained decoder + source-state pointer scan. small kana: F5=っ F6=ゃ F7=ゅ F8=ょ")
...
logLine("TRACE_DIALOGUE_V32_READY,frame=" .. emu.framecount() .. "," .. state())

local last_mode02_key = ""
local last_mode02_frame = -9999
local last_force_chain_frame = -9999
...
while true do
  ...
  if TRACE_ALIVE_FRAMES > 0 and (f % TRACE_ALIVE_FRAMES) == 0 then
    logLine("TRACE_DIALOGUE_V32_ALIVE,frame=" .. f .. "," .. state() .. ",source_state=" .. source_state_words())
  end

  -- v32: force chain dump periodically during long mode02 dialogue windows.
  if FORCE_CHAIN_EVERY_FRAMES > 0 and wram8(0x12AA) == 0x02 and (f - last_force_chain_frame) >= FORCE_CHAIN_EVERY_FRAMES then
    local best_force, decoded_force = find_best_mode02_candidate()
    if best_force ~= nil and best_force.score >= MIN_GOOD_SCORE then
      logLine(table.concat({
        "TRACE_DIALOGUE_V32_MODE02_FORCE_BEST",
        "frame=" .. f,
        "source=" .. best_force.name,
        "ptr=" .. ptr_text(best_force.bank, best_force.addr),
        "score=" .. tostring(best_force.score),
        "decoded=" .. trunc(best_force.decoded, 900),
        "end=" .. ptr_text(best_force.st.ptr_bank, best_force.st.ptr_addr) .. "/bitcnt" .. h2(best_force.st.bitcnt) .. "/err" .. tostring(best_force.err),
        "source_state=" .. source_state_words(),
        state()
      }, ","))
      if TRACE_MODE02_CANDIDATES then
        logLine(table.concat({
          "TRACE_DIALOGUE_V32_MODE02_FORCE_CANDIDATES",
          "frame=" .. f,
          "best=" .. best_force.name .. "@" .. ptr_text(best_force.bank, best_force.addr),
          "candidates=" .. compact_candidate_list(decoded_force)
        }, ","))
      end
      if TRACE_MODE02_CHAIN then
        local chain_force = decode_chain_candidate({name=best_force.name, bank=best_force.bank, addr=best_force.addr}, CHAIN_STREAMS)
        logLine(table.concat({
          "TRACE_DIALOGUE_V32_MODE02_FORCE_CHAIN",
          "frame=" .. f,
          "source=" .. best_force.name,
          "base=" .. ptr_text(best_force.bank, best_force.addr),
          "chain=" .. compact_chain_list(chain_force)
        }, ","))
      end
      last_force_chain_frame = f
    else
      logLine("TRACE_DIALOGUE_V32_MODE02_FORCE_NO_BEST,frame=" .. f .. "," .. state() .. ",source_state=" .. source_state_words())
      last_force_chain_frame = f
    end
  end
  ...
  if active_text() then
      ...
      if best ~= nil and best.score >= MIN_GOOD_SCORE then
        ...
        if best_key ~= last_mode02_key and (f - last_mode02_frame) > MODE02_DECODE_MIN_GAP then
          ...
          if TRACE_MODE02_CHAIN then
            ...
            logLine(table.concat({
              "TRACE_DIALOGUE_V32_MODE02_CHAIN",
              "frame=" .. f,
              "source=" .. best.name,
          ...
          if TRACE_VERBOSE_EVENTS then
            logLine(table.concat({
              "TRACE_DIALOGUE_V32_MODE02_EVENTS",
              "frame=" .. f,
              "source=" .. best.name,
              "ptr=" .. ptr_text(best.bank, best.addr),
    ...
    if TRACE_SCREEN_OBSERVED or TRACE_TOKENS then
      ...
      if TRACE_SCREEN_OBSERVED and decoded_buf ~= "" and (f - last_display_frame) > DISPLAY_IDLE_FLUSH_FRAMES then
        logLine(table.concat({
          "TRACE_DIALOGUE_V32_SCREEN_LINE",
          "frame=" .. f,
