...
| Goal9 VM/script | 100% | VM/event/script remain configuration-only for Goal13 visibility path. |
| Goal12 architecture | 100% | one-way VM/config → slot loop → append → dispatch/OAM pipeline stabilized. |
| Goal13 NPC/OAM | model 100% / evidence 99% | remaining work is constants, RAM addresses, and exact opcodes. |
...
The March handoffs had Goal13 much lower because the NPC throttling body was still distant from UI/descriptor/VRAM groundwork. The scheduled May outputs reframe Goal13 as a nearly closed bank87 branch/append problem while preserving earlier script/VM/store/ledger findings as separate subsystems.
