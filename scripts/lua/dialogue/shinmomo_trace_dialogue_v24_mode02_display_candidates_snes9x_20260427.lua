-- shinmomo_trace_dialogue_v24_mode02_display_candidates_snes9x_20260427.lua
-- v24: mode02/display compact + 装備会話候補復元版
--
-- v22で分かったこと:
...
local function reconstructed_candidate(s, hints)
  -- これは raw decode ではなく、display_decode断片 + ROM静的確認語 + 画面文脈の候補復元。
  -- 固定置換ではなく、別フィールド candidate として出す。
  if string.find(s, "そうを", 1, true) and string.find(s, "とのたか", 1, true) and string.find(s, "うびでる", 1, true) then
    return "「銀次の そうびを ととのえたか？ 銀次は 刀も そうびできるよ！」"
  end

  if string.find(s, "のそうび", 1, true) and string.find(s, "はちま", 1, true) and (string.find(s, "わじ", 1, true) or string.find(s, "じは", 1, true)) and string.find(s, "いっ", 1, true) then
    return "「銀次の そうびは 着流しだ！ はちまきと わらじは 桃太郎と いっしょだがな！」"
  end

  if string.find(s, "のそうび", 1, true) and string.find(hints or "", "銀次のそうび", 1, true) then
    return "銀次のそうび系メッセージ候補"
  end
...
logLine("shinmomo dialogue v24 mode02/display candidates loaded.")
logLine("domains=" .. table.concat(DOMAINS, "|") .. ", WRAM=" .. tostring(WRAM_DOMAIN) .. ", ROM=" .. tostring(ROM_DOMAIN))
logLine("TRACE_DIALOGUE_V24_READY,frame=" .. emu.framecount() .. "," .. state())
...
while true do
  ...
  if active_text() then
    ...
    if display_buf ~= "" and (f - last_display_frame) > 90 then
      ...
      logLine("TRACE_DIALOGUE_V24_DISPLAY_LINE,frame=" .. f .. ",display_decode=" .. display_buf .. ",display_hints=" .. hints .. ",reconstructed_candidate=" .. reconstructed_candidate(display_buf, hints) .. "," .. state())
      display_buf = ""
      last_display_key = ""
    end
  else
