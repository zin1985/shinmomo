-- shinmomo_trace_object_oam_compact_v2_snes9x_20260426.lua
-- Snes9x core fallback: object/OAM compact tracer v2
--
-- v2修正点:
--   OAM mirror 4byteを x,y,tile,attr として読む。
--   旧compact版は byte0をy扱いしていたため、hidden entry "00 E0 00 00" を
--   visibleとして数えてしまっていた。
--
-- 目的:
--   移動NPC/同行者/村NPCの active chain, visible slots, OAM visible entries を短く出す。
...
local function visibleSlots()
  ...
  for i = 0, 31 do
    ...
    -- ae5 != 0 を表示候補、または b25のgroupが20以外なら注目slotとして出す
...
local function oamVisible()
  ...
  for i = 0, 31 do
    ...
    -- 新桃のmirror上は x,y,tile,attr と読むのが自然。
    -- hidden entryは 00 E0 00 00 になり、y=E0。
    local x = wram8(base)
    local y = wram8(base + 1)
    local tile = wram8(base + 2)
    local attr = wram8(base + 3)
...
local function snapshotKey()
  ...
  return activeChain() .. "::" .. visibleSlots() .. "::" .. tostring(c) .. "::" .. logical1569() .. "::" .. oam
end

local function dump(tag)
  local count, oam = oamVisible()
  logLine(table.concat({
    "TRACE_OBJ_COMPACT_V2",
...
logLine("shinmomo compact object/OAM tracer v2 loaded.")
logLine("memory domain=" .. tostring(DOMAIN))
logLine("DUMP_INTERVAL=" .. tostring(DUMP_INTERVAL))
logLine("OAM layout interpreted as x,y,tile,attr; hidden is y=E0.")
