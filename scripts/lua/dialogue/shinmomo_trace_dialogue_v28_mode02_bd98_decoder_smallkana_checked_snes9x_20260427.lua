-- shinmomo_trace_dialogue_v28_mode02_bd98_decoder_smallkana_checked_snes9x_20260427.lua
-- v28: C0:BD98 mode02 bitstream decoder + small kana table checked
--
-- 目的:
...
local MODE02_DECODE_MIN_GAP = 90
...
logLine("shinmomo dialogue v28 mode02 BD98 decoder loaded. small kana checked: F5=っ F6=ゃ F7=ゅ F8=ょ")
...
logLine("TRACE_DIALOGUE_V28_READY,frame=" .. emu.framecount() .. "," .. state())
...
while true do
  ...
  if active_text() then
    ...
    if mode == 0x02 then
      -- Try both B1 pointer and $7E pointer. B1 is often the start/control pointer; $7E is the bitreader pointer.
      local b1_bank, b1_addr = src_ptr_b1()
      local e_bank, e_addr = ptr7e()

      local syms_b1, err_b1, st_b1 = decode_mode02_symbols_from(b1_bank, b1_addr, READ_SYMBOLS)
      local dec_b1, ev_b1 = render_symbols(syms_b1)

      local syms_7e, err_7e, st_7e = decode_mode02_symbols_from(e_bank, e_addr, READ_SYMBOLS)
      local dec_7e, ev_7e = render_symbols(syms_7e)

      local key = h2(b1_bank)..":"..h4(b1_addr)..":"..h2(e_bank)..":"..h4(e_addr)..":"..dec_b1..":"..dec_7e
      if key ~= last_mode02_key and (f - last_mode02_frame) > MODE02_DECODE_MIN_GAP then
        logLine(table.concat({
          "TRACE_DIALOGUE_V28_MODE02_DECODE",
          "frame=" .. f,
          "b1_ptr=" .. h2(b1_bank) .. ":" .. h4(b1_addr),
          "decoded_from_b1=" .. dec_b1,
          "events_b1=" .. ev_b1,
          "end_b1=" .. h2(st_b1.ptr_bank) .. ":" .. h4(st_b1.ptr_addr) .. "/bitcnt" .. h2(st_b1.bitcnt) .. "/err" .. tostring(err_b1),
          "ptr7E=" .. h2(e_bank) .. ":" .. h4(e_addr),
          "decoded_from_7E=" .. dec_7e,
          "events_7E=" .. ev_7e,
          "end_7E=" .. h2(st_7e.ptr_bank) .. ":" .. h4(st_7e.ptr_addr) .. "/bitcnt" .. h2(st_7e.bitcnt) .. "/err" .. tostring(err_7e),
