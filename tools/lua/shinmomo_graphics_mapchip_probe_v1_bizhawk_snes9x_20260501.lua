...
local OUT_DIR = "shinmomo_graphics_probe_out"
  ...
  for ch = 0, 7 do
    if (value & (1 << ch)) ~= 0 then
      ...
      append_text(dma_log_path, csv_line({
        f, ch, hex2(dmap), hex2(bbad), dma_target_label(bbad), hex6(src), size,
        hex2(ppu[0x2115]), hex2(ppu[0x2116]), hex2(ppu[0x2117]), hex2(ppu[0x2121]),
        "MDMAEN"
      }))
    end
  end
...
local function dump_snapshot(reason)
  ...
  append_text(manifest_path, csv_line({
    f, reason, dir, hex4(vh), hex4(ch), ppu_signature(),
...
write_text(manifest_path, csv_line({
  "frame","reason","dir","vram_hash","cgram_hash","ppu_signature",
...
write_text(dma_log_path, csv_line({
  "frame","channel","dmap","bbad","target","source","size",
  "vmain","vmaddl","vmaddh","cgadd","trigger"
}))
