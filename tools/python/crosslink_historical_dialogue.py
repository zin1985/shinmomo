        ...
        with path.open("r", encoding="utf-8-sig", errors="replace", newline="") as handle:
            for row_no, row in enumerate(csv.DictReader(handle), 2):
                ...
                if len(tokens) != len(fields):
                    continue
                decoded_text = (row.get("text") or "").strip()
                if tokens[0] != 0x7D or tokens[-1] != 0x00 or len(decoded_text) <= 4:
                    continue
                digest = hashlib.sha256(bytes(tokens)).hexdigest()
                out[digest].append({
                    "historical_file": rel,
                    "historical_row": row_no,
                    "historical_root": row.get("root", ""),
                    "historical_seg": row.get("seg", ""),
                    "historical_text_len": len(decoded_text),
...
def main() -> None:
    ...
    fields = [
        ...
        "historical_file", "historical_row", "historical_root", "historical_seg",
